    ...
    public function testItemWithFeatureMedia()
    {
        ...
        foreach ($images as $mediaId) {
            $client->request(
                ...
                [
                    'media_id' => $mediaId,
                    'media' => new UploadedFile(__DIR__.'/../app/Resources/test_file.jpg', 'test_file.jpg', 'image/jpeg', 3992, null, true),
                ]
            );
            ...
            $this->assertEquals(
                [
                    'media_id' => $mediaId,
                    'URL' => 'http://localhost/media/'.str_replace('/', '_', str_replace('jpg', 'jpeg', $mediaId)),
                    'media' => base64_encode(file_get_contents(__DIR__.'/../app/Resources/test_file.jpg')),
        ...
        $this->assertEquals(200, $client->getResponse()->getStatusCode());
        $content = json_decode($client->getResponse()->getContent(), true);
        self::assertArrayHasKey('featureMedia', $content);
